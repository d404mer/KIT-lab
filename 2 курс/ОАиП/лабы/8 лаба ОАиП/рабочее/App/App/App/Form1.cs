        ...
        private void PolygonDraw_Click(object sender, EventArgs e)
        {
            ...
            if (Convert.ToInt32(PolygonAmount.Text) != 0 && (PolygonAmount.Text) != "")
            {
                if (x >= 0 && x + w <= 468 && y >= 0 && y + w <= 462)
                {
                    if ((x - w) > 0 && (y - w) > 0)
                    {
                        Point position = new Point( (x + w/2), (y + h/2) );
                    ...
                    else MessageBox.Show("Фигура за пределами поля. Проверьте значения ширины и высоты");
                }
                else
                {
                    MessageBox.Show("Фигура за пределами поля. Проверьте значения");
                }
            }
        }
