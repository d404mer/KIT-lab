...
namespace ClassLib
{
    ...
    public class Special : Figure
    {
        ...
        public override void Draw(Graphics g)
        {
            ...
            int height = Convert.ToInt32(this.height * 0.67);

            g.DrawRectangle(Pens.Black, position.X, position.Y + height / 2, width, height);

            // Рисуем окно в доме
            float windowSize = Math.Min(width / 3, height / 3);
            float windowX = position.X + (width - windowSize) / 2;
            float windowY = position.Y + (height - windowSize) / 2;
            g.FillRectangle(Brushes.White, new RectangleF(windowX, windowY + height / 2, windowSize, windowSize));
