...
namespace ClassLib
{
    ...
    public class Special : Figure
    {
        ...
        public override void Draw(Graphics g)
        {
            ...
            // Глаз (круг)
            g.DrawEllipse(Pens.White, Convert.ToInt32( (position.X + height * 0.67)), Convert.ToInt32 ( (position.Y + height / 2) * 0.98), width/5, height/5);

            // Рот (прямоугольник)
            g.DrawRectangle(Pens.White, Convert.ToInt32( (position.X + height) * 0.59) , Convert.ToInt32( (position.Y + height) * 0.87), Convert.ToInt32 ( (width / 2) * 0.8), height / 11);
